    ...
    axios
      .get(
        "https://api.openweathermap.org/data/2.5/forecast/daily?q=" +
          this.state.city +
          "&cnt=5&units=metric&mode=json&appid=" +
          appid +
          "&apikey=" +
          apikey,
      )
  ...
  renderFiveDayForecast() {
    ...
    if (weatherData) {
      for (let i = 1; i < weatherData.list.length; i++) {
        ...
        let ele = (
          <li className="d-flex justify-content-between align-items-center" key={weather.dt}>
            <span className="w-50">
              {this.state.city}, {moment(weather.dt * 1000).format("ddd DD MMMM")}
            </span>
    ...
    return (
      <Fragment>
        ...
        <div className="weather-bottom">
          <ul className="list-inline mb-0">{this.renderFiveDayForecast()}</ul>
        </div>
      </Fragment>
    );
  }
