...
export default function BranchesDropDownWithArea({
  ...
  const deliveryids = delivery
    ?.filter((d) => +d.pickUpCityId == +choosenID)
    .map((dd) => +dd.dropOffCityId);
  const deliveryidsForDropof = delivery
    ?.filter((d) => +d.dropOffCityId == +choosenID)
    .map((dd) => +dd.pickUpCityId);
  // deliveryids.push(choosenID);
  const alldeliverdids = deliveryids.concat(deliveryidsForDropof);

  const options =
    allareas?.areas
      .filter((area) => !alldeliverdids.includes(+area.id))
      .map((x) => ({
