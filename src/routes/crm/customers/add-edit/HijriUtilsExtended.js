...
export default class HijriUtilsExtended extends HijriUtils {
  constructor() {
    ...
    this.locale = JSON.parse(localStorage.getItem("locale"))?.locale === "en" ? "en-Au" : "ar-Sa";
  }
  ...
  getDatePickerHeaderText = function (date) {
    return date.locale(this.locale).format("dddd, iDD iMMM");
  };
  getDateTimePickerHeaderText = function (date) {
    return date.locale(this.locale).format("iDD");
  };
  ...
  getWeekdays = () => {
    return JSON.parse(localStorage.getItem("locale"))?.locale === "en"
      ? ["sun", "mon", "tue", "wen", "thu", "fr", "sa"]
      : ["ح", "ات", "ث", "ار", "خ", "ج", "س"];
  };
  // Set month in Hijri.
