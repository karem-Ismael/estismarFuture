...
export default (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case ON_CHANGE_FEEDBACK_PAGE_TABS:
      if (action.payload === 1) {
        const plannedFeedbacks = state.allFeedbacks.filter(
          feedback => feedback.planned && !feedback.deleted,
        );
        return { ...state, feedbacks: plannedFeedbacks, selectedTab: action.payload };
      }
      if (action.payload === 2) {
        const progressFeedbacks = state.allFeedbacks.filter(
          feedback => feedback.inProgress && !feedback.deleted,
        );
      ...
      return {
        ...state,
        feedbacks: state.allFeedbacks.filter(feedback => !feedback.deleted),
        selectedTab: 0,
      };
    ...
    case ON_SEARCH_IDEA:
      if (action.payload === "") {
        ...
        const searchFeedbacks = state.allFeedbacks.filter(
          feedback => feedback.idea.toLowerCase().indexOf(action.payload.toLowerCase()) > -1,
        );
