...
function* signinUser({ payload }) {
  ...
  try {
    ...
    const getFromValue = searchParams.get("from");
    if (getFromValue) {
      history.push(`${getFromValue}`);
      // console.log(getFromValue);
    } else {
      history.push(`/es/dashboard/bookings`); // on carwah login
    }
  } catch (error) {
    console.error(error);
  }
...
function* signinUserWithFacebookAccount({ payload }) {
  try {
    ...
    if (signUpUser.message) {
      ...
      payload.push("/cw/dashboard/bookings"); // on login
    }
  } catch (error) {
    yield put(signinUserFailure(error));
  }
