  ...
  switch (action.type) {
    ...
    case SEARCH_EMAIL:
      if (action.payload === "") {
        ...
        const searchEmails = state.allEmail.filter(
          email =>
            !email.deleted &&
            email.email_subject.toLowerCase().indexOf(action.payload.toLowerCase()) > -1,
        );
    ...
    case GET_SENT_EMAILS:
      const sentEmails =
        state.allEmail && state.allEmail.filter(email => !email.deleted && email.folder === 1);
      return { ...state, emails: sentEmails, selectedFolder: 1 };

    // get inbox
    case GET_INBOX:
      const inbox =
        state.allEmail && state.allEmail.filter(email => !email.deleted && email.folder === 0);
      return { ...state, emails: inbox, selectedFolder: 0 };

    // get drafts
    case GET_DRAFTS_EMAILS:
      const drafts =
        state.allEmail && state.allEmail.filter(email => !email.deleted && email.folder === 2);
      return { ...state, emails: drafts, selectedFolder: 2 };

    // spam emails
    case GET_SPAM_EMAILS:
      const spamEmails =
        state.allEmail && state.allEmail.filter(email => !email.deleted && email.folder === 3);
    ...
    case FILTER_EMAILS_WITH_LABELS:
      let filterEmails = state.allEmail.filter(email =>
        email.email_labels.includes(action.payload.value),
      );
